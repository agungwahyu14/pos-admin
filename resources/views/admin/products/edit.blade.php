                        ...
                        <div class="mb-3">
                            ...
                            <select name="category_id" id="category_id" class="form-select bg-white rounded-3 p-2 @error('category_id') is-invalid @enderror" required onchange="updateSubCategories()">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" data-subcats="{{ $category->subCategories->toJson() }}" {{ old('category_id', $product->category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
...
<script>
    function updateSubCategories() {
        ...
        try {
            ...
            const oldSubCat = "{{ old('sub_category_id', $product->sub_category_id) }}";
