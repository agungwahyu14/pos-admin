...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h3 class="fw-bold text-dark">Edit Sub Category</h3>
            ...
            <div class="card-body p-4">
                <form action="{{ route('admin.subcategories.update', $subCategory->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" class="form-control bg-light rounded-3 px-3 py-2 @error('name') is-invalid @enderror" value="{{ old('name', $subCategory->name) }}" required>
                        @error('name')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="d-grid">
                        <button type="submit" class="btn btn-primary rounded-3 py-2 fw-medium">
                            <i class="bi bi-save me-2"></i> Update Sub Category
                        </button>
                    </div>
                </form>
            </div>
